    ...
    def sample(self, file_list):
        ...
        for file in file_list:
            ...
            if self.format == "npz":
                ...
                sample["begin_time"] = tdms_file.properties['GPSTimeStamp'].astype(datetime)
                sample["dx_m"] = tdms_file.properties['SpatialResolution[m]']
...
#                     data = data[:, ::downsample_ratio]
                ...
                data = np.vstack(tdms_file['Measurement'].channels()[::downsample_ratio]).T
                    
                if self.dt / sample["dt_s"] >= 2:
                    downsample_ratio = int(self.dt / sample["dt_s"])
                    sample["dt_s"] *= downsample_ratio
                    data = scipy.signal.decimate(data, downsample_ratio, axis=0).astype(np.float32)
    
                if self.highpass_filter > 0.0:
                    b, a = scipy.signal.butter(2, self.highpass_filter, "hp", fs=1./sample["dt_s"])
...
#                     data = fp["data"][:]  # nt x nx
                    ...
                    sample["begin_time"] = datetime.fromisoformat(fp["Acquisition/Raw[0]/RawDataTime"].attrs["PartStartTime"].decode('utf-8'))
                    sample["dx_m"] = fp["Acquisition"].attrs["SpatialSamplingInterval"]
